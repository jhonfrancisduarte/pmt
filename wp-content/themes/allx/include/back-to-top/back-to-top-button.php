...
add_action( 'customize_register', 'allx_back_to_top_customize_register' );
function allx_back_to_top_customize_register( $wp_customize ) {
	
		$wp_customize->selective_refresh->add_partial( 'activate_back_to_top', array(
			'selector' => '.cd-top',
			'render_callback' => 'allx_customize_partial_activate_back_to_top',
		) );	
...
***********************************************************************************/
		$wp_customize->add_section( 'back_to_top' , array(
			'title'       => __( 'Back To Top Button Options', 'allx' ),
			'priority'   => 98,
		) );
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'activate_back_to_top', array(
			'label'    => __( 'Activate Back To Top Button', 'allx' ),
			'section'  => 'back_to_top',
		...
		$wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'back_top_button_hover_color', array(
			'label' => __('Button Hover Color', 'allx'),        
			'section' => 'back_to_top',
...
* Back to top styles
*********************************************/ 	
add_action( 'wp_enqueue_scripts', 'allx_back_top_method' );	
function allx_back_top_method() {
