...
// For Categories Templates - Image Slider	
jQuery("body").ready(function(){
	
	var fileFrame;
	jQuery('.upload_image_button-cat').on('click', function(e){
		e.preventDefault();
		...
		fileFrame.on('select', function(){
			...
			if(attachment.url){					
				jQuery('input[name="mp_images_cat' + id + '"]').val(attachment.url);
				jQuery('#slider_preview_img_cat' + id).attr('src', attachment.url).fadeIn();
				jQuery(element).parent().find('.slider_remove_img').show();
			}
		});
	...
	jQuery('.slider_remove_img').on("click", function(e){
		...
		element = jQuery(this);
		id = jQuery(element).attr('data-id');
		image = jQuery('#slider_preview_img_cat' + id);
		
		jQuery('input[name="mp_images_cat' + id + '"]').val('');
		jQuery(image).fadeOut();
		jQuery(element).hide();
	});
