...
/*** Default List Style 1 */
	...
	$events_html .='<div class="ect-list-date">'.wp_kses_post($event_schedule);
	if (wp_is_mobile()) {
		$events_html .= '<div class="mobile-event-title">'.wp_kses_post($event_title).'</div>';
	}
	$events_html .= '</div></a>';
	$events_html.= wp_kses_post($share_buttons);
	...
	$events_html.=wp_kses_post($event_content).'<div class="custom-venue">'.wp_kses_post($venue_details_html).'</div>';
