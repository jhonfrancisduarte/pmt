...
$end_range_date = new DateTime($setting['ebec_date_range_end']);
    ...
    }else if($setting['ebec_type']== 'future'){
        if($setting['ebec_event_source'] == false ){
            $start_range_date = new DateTime('0000-01-01 00:00:00');
     ...
     return $date_style;
		 ...
		 function ebec_tribe_event_time( $display,$event ) {
			if ( tribe_event_is_multiday( $event ) ) { // multi-date event
				...
				if ( $display ) {
					printf( esc_html__( '%s - %s', 'ebec' ), esc_html($start_date), esc_html($end_date) );
				}
				else {
					return sprintf( esc_html__( '%s - %s', 'ebec' ), esc_html($start_date), esc_html($end_date) );
				}
			}
